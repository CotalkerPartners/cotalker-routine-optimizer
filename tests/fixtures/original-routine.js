                    ...
                    "stages" : [
                        {
                            ...
                            "next" : {
                                "CASE_A" : "iterar_propery",
                                "CASE_B" : "consolidated_logic",
                                "CASE_C" : "iterar_",
                                "CASE_D" : "change_state",
                                "CASE_E" : "",
                                "DEFAULT" : ""
                        ...
                        },
                        {
                            ...
                            "version" : "2.0.0",
                            "data" : {
                                "url" : "$JOIN#/#($ENV#BASEURL)#api#v2#properties#jsonpatch#($VALUE#asset)",
                                "method" : "PATCH",
                                "headers" : {
                                    "admin" : true
                                },
                            ...
                            "next" : {
                                "SUCCESS" : "patch_multiple_tasks_add",
                                "ERROR" : ""
                            },
                            "variables" : [
                        ...
                        },
                        {
                            ...
                            "version" : "2.0.3",
                            "data" : {
                                "src" : "const asset = data.asset;\nconst ossSubproperties = data.ossSubproperties || [];\n\n// 1. Normalización de entradas con fallbacks seguros\nconst answerOS = (Array.isArray(data.answerOS) && data.answerOS.length) ? data.answerOS : (data.answerOS_ || []);\nconst repuestosRaw = (Array.isArray(data.repuestos) && data.repuestos.length) ? data.repuestos : (data.repuestos_ || []);\nconst repuestos_os = Array.isArray(data.repuestos_os) ? data.repuestos_os : [];\n\nconst jsonPatch = [];\n\n// --- Validaciones base ---\nif (!asset?.schemaInstance?.ordenes_de_servicio) {\n    throw new Error(`El asset de id ${asset?._id} no posee en su schema el campo \"ordenes de servicio\"`);\n}\n\n// --- Parseo eficiente de repuestos ---\nconst repuestosParsed = repuestosRaw.map(r => {\n    if (typeof r === 'string') { try { return JSON.parse(r); } catch { return {}; } }\n    return (r && typeof r === 'object') ? r : {};\n});\n\nconst repuestosFromSchema = repuestosParsed.flatMap(r => \n    Array.isArray(r?.schemaInstance?.repuestos) ? r.schemaInstance.repuestos : []\n);\n\n// --- Uso de Sets para optimizar memoria y velocidad ---\nconst currentSubSet = new Set(Array.isArray(asset.subproperty) ? asset.subproperty : []);\nconst currentOSsSet = new Set(Array.isArray(asset.schemaInstance.ordenes_de_servicio) ? asset.schemaInstance.ordenes_de_servicio : []);\n\n// IDs únicos que llegan en esta ejecución\nconst nuevos_repuestos = [...new Set([...repuestosFromSchema, ...repuestos_os])].filter(id => id && typeof id === 'string' && id.trim());\nconst incomingOSs = [...new Set([...answerOS, ...ossSubproperties.map(e => e._id)])].filter(id => id && typeof id === 'string' && id.trim());\n\n// --- ESTRATEGIA: Solo parchear lo que NO existe ---\nconst OSsToPatch = incomingOSs.filter(id => !currentOSsSet.has(id));\nconst totalIncoming = [...new Set([...incomingOSs, ...nuevos_repuestos])];\nconst SubsToPatch = totalIncoming.filter(id => !currentSubSet.has(id));\n\n// Agregamos al patch solo los elementos faltantes usando el operador /- (append)\nOSsToPatch.forEach(id => {\n    jsonPatch.push({ op: \"add\", path: \"/schemaInstance/ordenes_de_servicio/-\", value: id });\n});\n\nSubsToPatch.forEach(id => {\n    jsonPatch.push({ op: \"add\", path: \"/subproperty/-\", value: id });\n});\n\n// --- Retorno optimizado: Solo enviamos los IDs NUEVOS a los siguientes pasos ---\nreturn {\n    jsonPatch, // Solo las operaciones 'add' para el asset principal\n    tasksOSsToUpdate: OSsToPatch, // Solo las OS que se están vinculando ahora (evita sobrecarga en el batch patch)\n    nuevos_repuestos: SubsToPatch // Solo los repuestos nuevos detectados\n};",
                                "data" : {
                        ...
                        },
                        {
                            ...
                            "data" : {
                                "data" : {
                                    ...
                                    "target_tsk_group" : "66a9165e8abf773cfe6efd8e",
                                    "assets" : "$OUTPUT#consolidated_remove_logic#data|tasksOSsToUpdate"
                                },
                                "code" : "patch_multiple_tasks"
                            },
                            "next" : {
                        ...
                        },
                        {
                            ...
                            "version" : "2.0.3",
                            "data" : {
                                "src" : "const ubicacionTecnico = String(data?.bodega ?? \"\");\nconst ordenServicio    = String(data?.os ?? \"\");\nconst comentario       = \"extra\";\n\n// Helpers\nconst toArr = v => Array.isArray(v) ? v : (v == null ? [] : [v]);\n\nconst items      = toArr(data?.item ?? data?.items ?? data?.codigo_repuesto);\nconst cantidades = toArr(data?.vcantidad ?? data?.cantidad ?? data?.cantidades);\n\nconst len = Math.max(items.length, cantidades.length);\nconst detalle = Array.from({ length: len }, (_, i) => ({\n  item: String(items[i] ?? \"\"),\n  cantidad: String(cantidades[i] ?? \"\"),\n  ubicacionTecnico\n})).filter(d => d.item); // quita este filter si quieres incluir vacíos\n\nconst result = {\n  comentario,\n  ubicacionTecnico,\n  ordenServicio,\n  detalle\n};\n\nreturn { result };\n",
                                "data" : {
                                    "bodega" : "$OUTPUT#get_user#data|data|extra|bodega",
                                    "os" : "$OUTPUT#ccjs_repuestos#data|numero_os",
                                    "cantidad" : "$OUTPUT#ccjs_descontar_stock#data|cantidad_utilizada",
                                    "item" : "$OUTPUT#ccjs_repuestos#data|id_repuestos"
                                }
                            },
                            "next" : {
                                "SUCCESS" : "call_bot_repuestos",
                                "ERROR" : ""
                            },
                            "variables" : [
                            ...
                            "isCritical" : false,
                            "risks" : "",
                            "_id" : ObjectId("688cc7192713219b9d5ec4e5"),
                            "customNetworkRequest" : [

                            ]
                        },
                        {
                            "key" : "call_bot_repuestos",
                            "name" : "NWRequest",
                            "version" : "2.0.0",
                            "data" : {
                                "url" : "$JOIN#/#($ENV#BASEURL)#api#v1#bots#run#(6798f0e963d284f6d0987150)",
                                "method" : "post",
                                "defaultAuth" : true,
                                "body" : {
                                    "data" : "$OUTPUT#ccjs_bot#data",
                                    "channelTask" : "$VALUE#channel",
                                    "repuesto_extra" : "si"
                                }
                            },
                            "next" : {
                                "SUCCESS" : "mensaje_sap_2",
                                "ERROR" : ""
                            },
                            "variables" : [
                            ...
                            "isCritical" : false,
                            "risks" : "",
                            "_id" : ObjectId("68839fbe12719bfc38b206eb"),
                            "customNetworkRequest" : [
                        ...
                        },
                        {
                            ...
                            "version" : null,
                            "data" : {
                                "left" : "$OUTPUT#iterar_no_cobrados#iterationResults|[size=>*]",
                                "right" : "0"
                            },
                            "next" : {
                                "IF" : "mensaje_metabase_3",
                                "ELSE" : "ccjs_no_cobrados"
                            },
                            "variables" : [
                            ...
                            "isCritical" : false,
                            "risks" : "",
                            "_id" : ObjectId("689c8e3994c491b6f0bee516"),
                            "customNetworkRequest" : [
                        ...
                        },
                        {
                            ...
                            "version" : "3.0.0",
                            "data" : {
                                "control" : "$VALUE#sentAnswer|data|[find=>identifier=ordenes_de_servicio_asp_fact_]|responses",
                                "target" : "os"
                            },
                            "next" : {
                                "STEP" : "ccjs_subproperty",
                                "DONE" : "get_os_subproperties_add"
                            },
                            "variables" : [

                            ],
                            "isCritical" : false,
                            "_id" : ObjectId("689b709e94c491b6f0b6e355"),
                            "customNetworkRequest" : [

                            ]
                        },
                        {
                            "key" : "ccjs_subproperty",
                            "name" : "CCJS",
                            "version" : "2.0.3",
                            "data" : {
                                "src" : "const orden_servicio_raw = data.orden_servicio;\n\n// Parsear a objeto\nlet orden_servicio_obj;\ntry {\n  orden_servicio_obj = typeof orden_servicio_raw === \"string\" \n    ? JSON.parse(orden_servicio_raw) \n    : orden_servicio_raw;\n} catch (e) {\n  throw new Error(\"orden_servicio no es un JSON válido\");\n}\n\n// Extraer valores\nconst rut_cliente = orden_servicio_obj?.schemaInstance?.rut_cliente || \"\";\nconst asset = orden_servicio_obj?.schemaInstance?.asset || \"\";\n\n// Retornar si lo necesitas\nreturn { rut_cliente, asset };",
                                "data" : {
                                    "orden_servicio" : "$VALUE#os"
                                }
                            },
                            "next" : {
                                "SUCCESS" : "get_property",
                                "ERROR" : ""
                            },
                            "variables" : [
                            ...
                            "isCritical" : false,
                            "risks" : "",
                            "_id" : ObjectId("696549a95eb44cff634e33ac"),
                            "customNetworkRequest" : [
                        ...
                        },
                        {
                            ...
                            "version" : "2.0.0",
                            "data" : {
                                "url" : "$JOIN#/#($ENV#BASEURL)#api#v2#properties#($OUTPUT#ccjs_subproperty#data|rut_cliente)",
                                "method" : "get",
                                "headers" : {
                                    "admin" : true
                                },
                            ...
                            "isCritical" : false,
                            "risks" : "",
                            "_id" : ObjectId("689c8fbe51459e940fcee449"),
                            "customNetworkRequest" : [

                            ]
                        },
                        {
                            "key" : "ccjs_add_subproperty",
                            "name" : "CCJS",
                            "version" : "2.0.3",
                            "data" : {
                                "src" : "const subproperty = Array.isArray(data.subproperty) ? data.subproperty : [];\nconst asset = data.asset;\n\n// Filtrar para quitar asset si existe\nconst result = subproperty.filter(id => id !== asset);\n\nreturn result;",
                                "data" : {
                                    "subproperty" : "$OUTPUT#get_property#data|data|subproperty",
                                    "asset" : "$OUTPUT#ccjs_subproperty#data|asset"
                                }
                            },
                            "next" : {
                                "SUCCESS" : "patch_subproperty",
                                "ERROR" : ""
                            },
                            "variables" : [
                            ...
                            "isCritical" : false,
                            "risks" : "",
                            "_id" : ObjectId("689c8fbe51459e940fcee44a"),
                            "customNetworkRequest" : [

                            ]
                        },
                        {
                            "key" : "patch_subproperty",
                            "name" : "NWRequest",
                            "version" : "2.0.0",
                            "data" : {
                                "url" : "$JOIN#/#($ENV#BASEURL)#api#v2#properties#jsonpatch#($OUTPUT#get_property#data|data|_id)",
                                "method" : "PATCH",
                                "headers" : {
                                    "admin" : true
                                },
                                ...
                                "body" : [
                                    {
                                        "op" : "add",
                                        "path" : "/subproperty",
                                        "value" : "$OUTPUT#ccjs_add_subproperty#data"
                                    }
                                ]
                            },
                            "next" : {
                            ...
                            "isCritical" : false,
                            "_id" : ObjectId("689b709e94c491b6f0b6e359"),
                            "customNetworkRequest" : [
                        ...
                        },
                        {
                            ...
                            "isCritical" : false,
                            "risks" : "",
                            "_id" : ObjectId("696549a95eb44cff634e33b1"),
                            "customNetworkRequest" : [

                            ]
                        },
                        {
                            "key" : "get_property_remove",
                            "name" : "NWRequest",
                            "version" : "2.0.0",
                            "data" : {
                                "url" : "$JOIN#/#($ENV#BASEURL)#api#v2#properties#($OUTPUT#ccjs_subproperty_remove#data|rut_cliente)",
                                "method" : "get",
                                "headers" : {
                                    "admin" : true
                                },
                            ...
                            "isCritical" : false,
                            "risks" : "",
                            "_id" : ObjectId("696549a95eb44cff634e33b2"),
                            "customNetworkRequest" : [
                        ...
                        },
                        {
                            ...
                            "version" : "2.0.3",
                            "data" : {
                                "src" : "const subproperty = Array.isArray(data.subproperty) ? data.subproperty : [];\nconst asset = data.asset;\n\n// Agregar asset solo si no existe ya\nif (!subproperty.includes(asset)) {\n  subproperty.push(asset);\n}\n\nreturn subproperty;",
                                "data" : {
                                    "subproperty" : "$OUTPUT#get_property_remove#data|data|subproperty",
                                    "asset" : "$OUTPUT#ccjs_subproperty_remove#data|asset"
                                }
                            },
                            "next" : {
                                "SUCCESS" : "patch_subproperty_remove",
                                "ERROR" : ""
                            },
                            "variables" : [
                            ...
                            "isCritical" : false,
                            "risks" : "",
                            "_id" : ObjectId("696549a95eb44cff634e33b3"),
                            "customNetworkRequest" : [

                            ]
                        },
                        {
                            "key" : "patch_subproperty_remove",
                            "name" : "NWRequest",
                            "version" : "2.0.0",
                            "data" : {
                                "url" : "$JOIN#/#($ENV#BASEURL)#api#v2#properties#jsonpatch#($OUTPUT#get_property_remove#data|data|_id)",
                                "method" : "PATCH",
                                "headers" : {
                                    "admin" : true
                                },
                            ...
                            "isCritical" : false,
                            "risks" : "",
                            "_id" : ObjectId("696549a95eb44cff634e33b4"),
                            "customNetworkRequest" : [
                        ...
                        },
                        {
                            ...
                            "version" : "2.0.3",
                            "data" : {
                                "src" : "const asset = data.asset;\nconst ossToRemoveRaw = (Array.isArray(data.ossToRemove) ? data.ossToRemove : []);\n\n// Mapas para efectos secundarios (Clientes)\nconst clientToAssetMap = {};\nconst uniqueClients = new Set();\n\n// Normalizar lista de OS a eliminar (IDs)\nconst idsToRemove = new Set(ossToRemoveRaw.map(os => {\n  const obj = (typeof os === 'string') ? JSON.parse(os) : os;\n  const id = obj?._id || obj?.$id;\n  \n  // Extraer datos para actualizar el cliente (side-effect)\n  const rc = obj?.schemaInstance?.rut_cliente;\n  const as = obj?.schemaInstance?.asset;\n  if (rc && as) {\n    if (!clientToAssetMap[rc]) clientToAssetMap[rc] = new Set();\n    clientToAssetMap[rc].add(as);\n    uniqueClients.add(rc);\n  }\n  \n  return id;\n}).filter(id => id && typeof id === 'string'));\n\nconst jsonPatch = [];\n\nif (!asset) {\n  throw new Error(\"No se recibió el asset de facturación para procesar el retiro.\");\n}\n\n// --- 1. Identificar Repuestos a eliminar ---\nconst repuestosToRemove = new Set();\nossToRemoveRaw.forEach(os => {\n  const obj = (typeof os === 'string') ? JSON.parse(os) : os;\n  const schema = obj?.schemaInstance || {};\n  if (Array.isArray(schema.repuestos)) {\n    schema.repuestos.forEach(r => repuestosToRemove.add(r));\n  }\n});\n\n// --- 2. Generar Patch para 'schemaInstance/ordenes_de_servicio' ---\nconst currentOSs = Array.isArray(asset.schemaInstance?.ordenes_de_servicio) ? asset.schemaInstance.ordenes_de_servicio : [];\nconst osIndices = [];\ncurrentOSs.forEach((id, index) => {\n  if (idsToRemove.has(id)) osIndices.unshift(index);\n});\nosIndices.forEach(idx => {\n  jsonPatch.push({ op: \"remove\", path: `/schemaInstance/ordenes_de_servicio/${idx}` });\n});\n\n// --- 3. Generar Patch para 'subproperty' ---\nconst currentSubs = Array.isArray(asset.subproperty) ? asset.subproperty : [];\nconst subIndices = [];\ncurrentSubs.forEach((id, index) => {\n  if (idsToRemove.has(id) || repuestosToRemove.has(id)) {\n    subIndices.unshift(index);\n  }\n});\nsubIndices.forEach(idx => {\n  jsonPatch.push({ op: \"remove\", path: `/subproperty/${idx}` });\n});\n\nreturn {\n  jsonPatch,\n  tasksOSsToRemove: [...idsToRemove],\n  repuestosRemoved: [...repuestosToRemove],\n  uniqueClients: [...uniqueClients],\n  clientMapping: Object.fromEntries(Object.entries(clientToAssetMap).map(([k,v]) => [k, [...v]]))\n};",
                                "data" : {
                                    "asset" : "$VALUE#asset",
                        ...
                        },
                        {
                            ...
                            "data" : {
                                "url" : "$JOIN#/#($ENV#BASEURL)#api#v2#properties#jsonpatch#($VALUE#asset)",
                                "method" : "PATCH",
                                "body" : "$OUTPUT#consolidated_logic#data|jsonPatch"
                            },
                            "next" : {
                                "SUCCESS" : "find_clients",
                                "ERROR" : ""
                            },
                            "variables" : [
                        ...
                        },
                        {
                            ...
                            "version" : "2.0.0",
                            "data" : {
                                "url" : "$JOIN#/#($ENV#BASEURL)#api#properties#find",
                                "method" : "POST",
                                "body" : {
                                    "_id" : {
                                        "$in" : "$OUTPUT#consolidated_logic#data|uniqueClients"
                                    }
                                }
                            },
                            "next" : {
                                "SUCCESS" : "format_client_patches",
                                "ERROR" : ""
                            },
                            "variables" : [
                            ...
                            "isCritical" : false,
                            "_id" : ObjectId("696549a95eb44cff634e33b8"),
                            "customNetworkRequest" : [

                            ]
                        },
                        {
                            "key" : "format_client_patches",
                            "name" : "CCJS",
                            "version" : "2.0.3",
                            "data" : {
                                "src" : "const clients = Array.isArray(data.clients) ? data.clients : [];\nconst mapping = data.clientMapping || {};\nconst multiPatch = [];\n\nclients.forEach(client => {\n  const clientId = client._id;\n  const assetsToAdd = mapping[clientId] || [];\n  const currentSubs = new Set(Array.isArray(client.subproperty) ? client.subproperty : []);\n  \n  // Identificar qué IDs de 'asset' no están en la subpropiedad del cliente\n  const delta = assetsToAdd.filter(id => id && !currentSubs.has(id));\n  \n  if (delta.length > 0) {\n    // Generamos las operaciones de patch para este cliente\n    const ops = delta.map(id => ({ \n      op: \"add\", \n      path: \"/subproperty/-\", \n      value: id \n    }));\n    \n    multiPatch.push({\n      _id: clientId,\n      ops: ops\n    });\n  }\n});\n\nreturn { multiPatch };\n",
                                "data" : {
                                    "clients" : "$OUTPUT#find_clients#data",
                                    "clientMapping" : "$OUTPUT#consolidated_logic#data|clientMapping"
                                }
                            },
                            "next" : {
                                "SUCCESS" : "patch_clients_bulk",
                                "ERROR" : ""
                            },
                            "variables" : [
                            ...
                            "isCritical" : false,
                            "_id" : ObjectId("696549a95eb44cff634e33b9"),
                            "customNetworkRequest" : [

                            ]
                        },
                        {
                            "key" : "patch_clients_bulk",
                            "name" : "NWRequest",
                            "version" : "2.0.0",
                            "data" : {
                                "url" : "$JOIN#/#($ENV#BASEURL)#api#v2#properties#jsonpatch#multi",
                                "method" : "POST",
                                "body" : "$OUTPUT#format_client_patches#data|multiPatch"
                            },
                            "next" : {
                                "SUCCESS" : "patch_multiple_tasks_delete",
                                "ERROR" : ""
